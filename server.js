...
// Load default cookie from cookies.txt if available
let defaultCookie = null;
try {
  const cookiePath = path.join(__dirname, "cookies.txt");
  if (fs.existsSync(cookiePath)) {
    defaultCookie = fs.readFileSync(cookiePath, "utf8").trim();
    console.log("[Server] Loaded default cookie from cookies.txt");
  }
} catch (err) {
  console.log(
    "[Server] No default cookie file found or error reading:",
    err.message
  );
}
...
app.use(async (req, res, next) => {
  ...
  if (req.path !== "/manifest.json" && !req.path.endsWith("/manifest.json")) {
    ...
    const paramSegments =
      streamIndex !== -1 ? pathSegments.slice(0, streamIndex) : pathSegments;
    ...
    if (Object.keys(pathParams).length > 0) {
      ...
      if (maskedPathParams.scraper_api_key)
        maskedPathParams.scraper_api_key = "[MASKED]";
      console.log(
        `[server.js] Extracted path parameters: ${JSON.stringify(
          maskedPathParams
        )}`
      );
    }
  ...
  if (cookie) {
    try {
      ...
      console.error(
        `[server.js] Error decoding cookie from request: ${cookie}`,
        e.message
      );
    }
  }
  ...
  if (minQualities) {
    try {
      ...
      console.error(
        `[server.js] Error parsing min_qualities from request: ${minQualities}`,
        e.message
      );
    }
  }
  if (scraperApiKey) {
    try {
      global.currentRequestConfig.scraper_api_key =
        decodeURIComponent(scraperApiKey);
    } catch (e) {
      console.error(
        `[server.js] Error decoding scraper_api_key from request: ${scraperApiKey}`,
        e.message
      );
    }
  }
  ...
  if (cookiesParam) {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        global.currentRequestConfig.cookies = parsed.filter(
          (c) => typeof c === "string" && c.trim().length > 0
        );
      }
    } catch (e) {
      console.error(
        `[server.js] Error parsing cookies array from request: ${cookiesParam}`,
        e.message
      );
    }
  }
  if (excludeCodecs) {
    try {
      ...
      global.currentRequestConfig.excludeCodecs =
        JSON.parse(decodedExcludeCodecs);
    } catch (e) {
      console.error(
        `[server.js] Error parsing exclude_codecs from request: ${excludeCodecs}`,
        e.message
      );
    }
  }

  // Use default cookie from cookies.txt if no cookie provided in request
  if (
    !global.currentRequestConfig.cookie &&
    !global.currentRequestConfig.cookies &&
    defaultCookie
  ) {
    global.currentRequestConfig.cookie = defaultCookie;
    console.log("[Server] Using default cookie from cookies.txt");
  }
  ...
  if (Object.keys(global.currentRequestConfig).length > 0) {
    ...
    if (configForLog.scraper_api_key)
      configForLog.scraper_api_key = "[PRESENT: ****]";
    if (configForLog.cookies)
      configForLog.cookies = `[${configForLog.cookies.length} cookies]`;

    console.log(
      `[server.js] Set global.currentRequestConfig for this request: ${JSON.stringify(
        configForLog
      )}`
    );
    ...
    if (req.path.includes("stream") || req.url.includes("stream")) {
      const maskedUrlForLog = (req.url || "").replace(
        /(cookie=|cookies=|scraper_api_key=)[^&/]+/g,
        "$1[MASKED]"
      );
      ...
      console.log(
        `[server.js] STREAM REQUEST detected - URL: ${maskedUrlForLog}`
      );
      console.log(
        `[server.js] STREAM REQUEST parameters - Path: ${JSON.stringify(
          maskedPathParamsForLog
        )}, Query: ${JSON.stringify(maskedQueryForLog)}`
      );
    }
  }
...
app.post("/api/validate-cookie", async (req, res) => {
  ...
  if (!cookie || typeof cookie !== "string" || cookie.trim() === "") {
    return res
      .status(400)
      .json({ isValid: false, message: "Cookie is required." });
  }
  ...
  console.log(
    `[validate-cookie] Testing cookie: ${cookie.substring(0, 15)}...`
  );
  ...
  try {
    ...
    try {
      const primaryResponse = await axios.get(
        "https://febbox.andresdev.org/movie/950396",
        {
          headers: {
      ...
      if (primaryResponse.status === 429) {
        console.warn(
          "[validate-cookie] Primary API returned 429 (rate limited)."
        );
        return res
          .status(429)
          .json({ isValid: false, message: "Rate limited by validation API" });
      }

      if (
        primaryResponse.status === 200 &&
        primaryResponse.data &&
        Array.isArray(primaryResponse.data.sources)
      ) {
        const hasSources = primaryResponse.data.sources.length > 0;
        if (hasSources) {
          console.log(
            "[validate-cookie] PRIMARY SUCCESS: Sources present from febbox.andresdev API."
          );
          return res.json({
            isValid: true,
            message: "Cookie valid (primary API).",
          });
        }
      }

      console.log(
        `[validate-cookie] PRIMARY did not confirm validity (status ${primaryResponse.status}). Falling back...`
      );
    } catch (primaryErr) {
      console.warn(
        `[validate-cookie] PRIMARY API error, falling back: ${primaryErr.message}`
      );
    ...
    if (
      sharePageResponse.status !== 200 ||
      !sharePageResponse.data ||
      typeof sharePageResponse.data !== "string"
    ) {
      let message = "Failed to load test share page.";
      if (
        sharePageResponse.status === 301 ||
        sharePageResponse.status === 302
      ) {
        const location = sharePageResponse.headers.location || "";
        message =
          location.includes("/login") || location.includes("/passport")
    ...
    const fidMatches = htmlContent.match(
      /div\s+class="file[^"]*"\s+data-id="(\d+)"/gi
    );
    if (fidMatches) {
    ...
    let shareKeyMatch = htmlContent.match(
      /var\s+share_key\s*=\s*['"]([a-zA-Z0-9-]+)['"]/
    );
    if (!shareKeyMatch)
      shareKeyMatch = htmlContent.match(/share_key:\s*['"]([a-zA-Z0-9-]+)['"]/);
    if (!shareKeyMatch)
      shareKeyMatch = htmlContent.match(
        /shareid\s*=\s*['"]([a-zA-Z0-9-]+)['"]/
      );
    if (shareKeyMatch) {
    ...
    if (!fid || !shareKey) {
      const message =
        "Could not extract necessary FID or Share Key from the test page.";
      console.log(
        `[validate-cookie] Step 2 FAILED: ${message} (FID: ${fid}, ShareKey: ${shareKey})`
      );
      return res.json({ isValid: false, message });
    }
    console.log(
      `[validate-cookie] Step 2 SUCCESS: Extracted FID: ${fid}, ShareKey: ${shareKey}`
    );

    // Step 3: Attempt to fetch player sources with extracted FID and Share Key
    console.log(
      `[validate-cookie] Step 3: Accessing player with FID ${fid}, ShareKey ${shareKey}`
    );
    ...
    if (playerResponse.data && typeof playerResponse.data === "string") {
      if (
        playerResponse.data.includes("sources = [") ||
        (playerResponse.data.includes("http") &&
          (playerResponse.data.includes(".mp4") ||
            playerResponse.data.includes(".m3u8")))
      ) {
        console.log(
          "[validate-cookie] Step 3 SUCCESS: Player sources found. Cookie is VALID."
        );
        return res.json({
          isValid: true,
          message: "Cookie successfully fetched video sources.",
        });
      }
    }
    ...
    if (playerResponse.data && typeof playerResponse.data === "object") {
      ...
      playerResponseMessage =
        playerResponse.data.msg ||
        JSON.stringify(playerResponse.data).substring(0, 100);
    } else if (playerResponse.data && typeof playerResponse.data === "string") {
      playerResponseMessage = `Player returned: ${playerResponse.data.substring(
        0,
        100
      )}...`;
    }
    ...
    console.error(
      "[validate-cookie] Error during validation process:",
      error.message
    );
...
app.use((req, res, next) => {
  ...
  if (req.path === "/manifest.json" || req.path === "/stream") {
    console.log(
      `Request to ${req.path}: User cookie provided: ${
        userCookie ? "Yes (length: " + userCookie.length + ")" : "No"
      }`
    );
    console.log(
      `Request to ${req.path}: Region preference: ${
        userRegionPreference || "None"
      }`
    );
  ...
  if (req.path === "/stream" && req.body && req.body.config) {
    ...
    if (!global.currentRequestRegionPreference && req.body.config.region) {
      global.currentRequestRegionPreference =
        req.body.config.region.toUpperCase();
    }
...
app.use((req, res, next) => {
  ...
  if (pathMatch) {
    ...
    console.log(
      `[server.js] Detected path-based stream request: ${req.path.replace(
        /(cookie=|cookies=|scraper_api_key=)[^/&]+/g,
        "$1[MASKED]"
      )}`
    );
    ...
    console.log(
      `[server.js] Path params: ${maskedParamsStr}, Type: ${type}, ID: ${id}`
    );
...
app.get("*manifest.json", async (req, res) => {
  try {
    ...
    if (userCookie) {
      ...
      const cookieConfigIndex = personalizedManifest.config.findIndex(
        (c) => c.key === "userFebBoxCookie"
      );
      const cookieValueForManifest = userCookie.startsWith("ui=")
        ? userCookie
        : `ui=${userCookie}`;
      if (cookieConfigIndex > -1) {
        personalizedManifest.config[cookieConfigIndex].default =
          cookieValueForManifest;
      } else {
    ...
    if (userRegion) {
      ...
      const regionConfigIndex = personalizedManifest.config.findIndex(
        (c) => c.key === "userRegionChoice"
      );
      ...
      console.log(
        `[Manifest] Region ${userRegion} applied to name, description, and config.`
      );
    }
    ...
    if (userProviders) {
      isPersonalized = true;
      const providersString = userProviders
        .split(",")
        .map((p) => p.trim().toLowerCase())
        .join(",");
      const providersConfigIndex = personalizedManifest.config.findIndex(
        (c) => c.key === "selectedProviders"
      );
      if (providersConfigIndex > -1) {
        personalizedManifest.config[providersConfigIndex].default =
          providersString;
      } else {
        personalizedManifest.config.push({
      ...
      console.log(
        `[Manifest] Providers (${providersString}) will be part of the config.`
      );
    }
    ...
    if (global.currentRequestConfig.minQualities) {
      isPersonalized = true;
      const minQualitiesString = JSON.stringify(
        global.currentRequestConfig.minQualities
      );
      const minQualitiesConfigIndex = personalizedManifest.config.findIndex(
        (c) => c.key === "minQualities"
      );
      if (minQualitiesConfigIndex > -1) {
        personalizedManifest.config[minQualitiesConfigIndex].default =
          minQualitiesString;
      } else {
        personalizedManifest.config.push({
          key: "minQualities",
      ...
      console.log(
        `[Manifest] Minimum qualities (${minQualitiesString}) will be part of the config.`
      );
    }
    ...
    if (isPersonalized) {
      ...
      let identifierSource =
        (userCookie || "nocookie") +
        "-" +
        (userRegion || "noregion") +
        "-" +
        (userProviders || "allproviders");
      const hash = crypto
        .createHash("sha1")
        .update(identifierSource)
        .digest("hex")
        .substring(0, 8);
      personalizedManifest.id = `${originalManifest.id}_${hash}`;
      console.log(
        `[Manifest] Personalized manifest ID set to: ${personalizedManifest.id}`
      );
    } else {
...
const createCustomRouter = (currentAddonInterface) => {
  ...
  return (req, res, next) => {
    ...
    if (req.userCookie) {
      ...
      console.log(
        `[server.js] Setting user cookie for this request (length: ${req.userCookie.length})`
      );
    }
    ...
    if (req.userRegion) {
      ...
      console.log(
        `[server.js] Setting region preference for this request: ${req.userRegion}`
      );
    } else {
      // Reset for safety
    ...
    res.on("finish", () => {
      ...
      global.currentRequestRegionPreference = null;
      console.log(
        `[server.js] Cleared cookie and region preference after request processing`
      );
    });
...
app.use((req, res, next) => {
  ...
  res.end = function () {
    ...
    if (
      ...
      console.log(
        "Cleaning up global cookie and region preference after request"
      );
      global.currentRequestUserCookie = null;
      global.currentRequestRegionPreference = null;
    }
...
app.listen(PORT, () => {
  console.log(
    `Nuvio Streams Addon landing page available at http://localhost:${PORT}`
  );
  ...
  console.log(
    `To generate a personalized manifest, append ?cookie=YOUR_URL_ENCODED_COOKIE to the manifest URL.`
  );
  console.log(
    `Example: http://localhost:${PORT}/manifest.json?cookie=ui%3Dyourcookievalue`
  );
  console.log(
    `Install example: stremio://localhost:${PORT}/manifest.json?cookie=ui%3Dyourcookievalue`
  );
});
