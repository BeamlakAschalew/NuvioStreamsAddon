...
console.log(
  `Current DISABLE_CACHE value: '${
    process.env.DISABLE_CACHE
  }' (Type: ${typeof process.env.DISABLE_CACHE})`
);
...
if (process.env.USE_REDIS_CACHE === "true") {
  ...
  try {
    ...
    console.log(
      `[Showbox Cache] Initializing Redis in Showbox.js. REDIS_URL from env: ${
        process.env.REDIS_URL ? "exists and has value" : "MISSING or empty"
      }`
    );
    if (!process.env.REDIS_URL) {
      throw new Error(
        "REDIS_URL environment variable is not set or is empty for Showbox Redis."
      );
    }
      ...
      redisKeepAliveInterval = setInterval(() => {
        if (redisClient && redisClient.status === "ready") {
          redisClient.ping((err) => {
            // No need to log success, just handle error
            if (err) {
              console.error(
                "[Showbox Redis Keep-Alive] Ping failed:",
                err.message
              );
            }
    ...
    redisClient.on("error", (err) => {
      ...
      console.error(
        `[Showbox Redis Error] ${err.message}. Falling back to FS. Showbox Redis Opts Host: ${redisClient?.options?.host}, Port: ${redisClient?.options?.port}`
      );
    ...
    // Catch errors from new Redis() or the explicit REDIS_URL check
    console.error(
      `[Showbox Cache] Failed to initialize Redis client: ${initError.message}`
    );
    redisClient = null; // Ensure client is null if initialization fails
  }
} else {
  console.log(
    "[Showbox Cache] Redis cache is disabled (USE_REDIS_CACHE is not 'true'). Using file system cache only."
  );
}
...
const loadFallbackCookies = async () => {
  try {
    ...
    console.log(
      `Loaded ${loadedCookies.length} fallback cookies from cookies.txt`
    );
    return loadedCookies;
  } catch (error) {
    console.warn(
      `Warning: Could not load fallback cookies from cookies.txt: ${error.message}`
    );
    return [];
  }
};
...
const getCookieForRequest = async (
  ...
  if (regionPreference) {
    if (
      global.regionAvailabilityStatus &&
      global.regionAvailabilityStatus[regionPreference] === false
    ) {
      usingFallback = true;
      for (const fallbackRegion of US_FALLBACK_REGIONS) {
        if (
          !global.regionAvailabilityStatus ||
          global.regionAvailabilityStatus[fallbackRegion] !== false
        ) {
          detectedOssGroup = fallbackRegion;
          console.log(
            `[CookieManager] Region ${regionPreference} known to be unavailable, using fallback US region: ${detectedOssGroup}`
          );
          break;
        }
      }
      if (!detectedOssGroup) {
        ...
        console.log(
          `[CookieManager] All fallback regions unavailable, trying original region: ${detectedOssGroup} anyway`
        );
      }
    } else {
      console.log(
        `[CookieManager] Using explicit region from parameter: ${regionPreference}`
      );
      detectedOssGroup = regionPreference;
    }
  } else {
    console.log(
      `[CookieManager] No explicit region preference provided, using default: ${DEFAULT_OSS_REGION}`
    );
  ...
  if (!baseCookieToUse && global.currentRequestConfig) {
    try {
      ...
      const providedSingle =
        typeof global.currentRequestConfig.cookie === "string" &&
        global.currentRequestConfig.cookie.trim().length > 0
      ...
      if (candidates.length > 0) {
        ...
        if (best && best.ok) {
          ...
          console.log(
            `[CookieManager] Selected best cookie by remaining quota: ${best.remainingMB} MB`
          );
        }
      }
    } catch (e) {
      console.warn(
        `[CookieManager] Multi-cookie selection error: ${e.message}. Falling back to single-cookie flow.`
      );
    }
  }
  ...
  if (
    ...
    global.currentRequestConfig.chosenFebboxBaseCookieForRequest
  ) {
    console.log(
      `[CookieManager] Re-using request-cached base cookie for this cycle.`
    );
    baseCookieToUse =
      global.currentRequestConfig.chosenFebboxBaseCookieForRequest;
    ...
    if (userCookie) {
      console.log(
        "[CookieManager] Using user-supplied cookie passed to function for this cycle."
      );
    ...
    // 2. Prioritize user-supplied cookie from global (fallback for backward compatibility)
      ...
      console.log(
        "[CookieManager] Using user-supplied cookie from global state (legacy mode) for this cycle."
      );
    ...
    // 3. Fallback to rotating cookies from cookies.txt
      ...
      if (cookieCache && cookieCache.length > 0) {
        ...
        console.log(
          `[CookieManager] Selected fallback cookie ${currentFallbackIndexDisplay} of ${cookieCache.length} from cookies.txt for this request cycle.`
        );
        baseCookieToUse = fallbackCookie;
      } else {
        console.log(
          "[CookieManager] No user-supplied or fallback cookies available for this cycle."
        );
  ...
  if (baseCookieToUse && baseCookieToUse.trim() !== "") {
    if (detectedOssGroup) {
      console.log(
        `[CookieManager] Applying region: ${detectedOssGroup} to chosen base cookie.`
      );
    ...
    if (detectedOssGroup) {
      console.log(
        `[CookieManager] No base cookie available, using only region: ${detectedOssGroup} as cookie.`
      );
      return `oss_group=${detectedOssGroup}`;
    }
    console.log(
      `[CookieManager] No base cookie and no region detected. Returning empty cookie string.`
    );
...
const fetchStreamSize = async (url) => {
  ...
  if (cachedSize !== null) {
    // Check for null specifically, as 'Unknown size' is a valid cached string
...
const FEBBOX_FILE_SHARE_LIST_URL =
...
const SHOWBOX_PROXY_URLS = (
  process.env.SHOWBOX_PROXY_URLS ||
...
const FEBBOX_PROXY_URLS = (
  process.env.FEBBOX_PROXY_URLS ||
  process.env.FEBBOX_PROXY_URL_VALUE ||
  ""
)
  .split(",")
  .map((s) => s.trim())
...
const ensureCacheDir = async (dirPath) => {
  try {
    ...
    if (error.code !== "EEXIST") {
      console.warn(
        `Warning: Could not create cache directory ${dirPath}: ${error.message}`
      );
    }
  }
};
...
const getFromCache = async (cacheKey, subDir = "") => {
  if (process.env.DISABLE_CACHE === "true") {
    console.log(
      `  CACHE DISABLED: Skipping read for ${path.join(subDir, cacheKey)}`
    );
    return null;
  }
  ...
  if (redisClient && redisClient.status === "ready") {
    try {
      ...
      console.warn(
        `  REDIS CACHE READ ERROR for ${fullCacheKey}: ${redisError.message}. Falling back to file system cache.`
      );
    }
  } else if (redisClient) {
    console.log(
      `  Redis client not ready (status: ${redisClient.status}). Skipping Redis read for ${fullCacheKey}, trying file system.`
    );
  }
  ...
  try {
    ...
    if (redisClient && redisClient.status === "ready") {
      try {
        ...
        console.log(
          `  Populated REDIS CACHE from FILE SYSTEM for: ${fullCacheKey} (TTL: ${
            ttlSeconds / 3600
          }h)`
        );
      } catch (redisSetError) {
        console.warn(
          `  REDIS CACHE SET ERROR (after file read) for ${fullCacheKey}: ${redisSetError.message}`
        );
      }
    }
    try {
    ...
    if (error.code !== "ENOENT") {
      console.warn(
        `  FILE SYSTEM CACHE READ ERROR for ${cacheKey}: ${error.message}`
      );
    } else {
      console.log(
        `  FILE SYSTEM CACHE MISS for: ${path.join(subDir, cacheKey)}`
      );
    }
    return null;
  }
...
const saveToCache = async (cacheKey, content, subDir = "") => {
  if (process.env.DISABLE_CACHE === "true") {
    console.log(
      `  CACHE DISABLED: Skipping write for ${path.join(subDir, cacheKey)}`
    );
    return;
  }
  ...
  if (redisClient && redisClient.status === "ready") {
    try {
      ...
      console.log(
        `  SAVED TO REDIS CACHE: ${fullCacheKey} (TTL: ${ttlSeconds / 3600}h)`
      );
    } catch (redisError) {
      console.warn(
        `  REDIS CACHE WRITE ERROR for ${fullCacheKey}: ${redisError.message}. Proceeding with file system cache.`
      );
    }
  } else if (redisClient) {
    console.log(
      `  Redis client not ready (status: ${redisClient.status}). Skipping Redis write for ${fullCacheKey}.`
    );
  }
  ...
  try {
    ...
    console.warn(
      `  FILE SYSTEM CACHE WRITE ERROR for ${cacheKey}: ${error.message}`
    );
  }
};
...
const validateShowboxTitle = (
  ...
  if (!showboxPageTitle || !tmdbMainTitle) {
    console.log(
      `  [Validation] Missing titles. SB: "${showboxPageTitle}", TMDB Main: "${tmdbMainTitle}"`
    );
    return false;
  }
  ...
  const titlesToCompare = [
    tmdbMainTitle,
    tmdbOriginalTitle,
    ...tmdbAlternativeTitles.map((alt) => alt.title),
  ]
  ...
  if (
    titlesToCompare.some(
      (normTmdbTitle) =>
        normalizedPageTitle === normTmdbTitle && normTmdbTitle.length > 0
    )
  ) {
    console.log(
      `  [Validation] SUCCESS: Exact match. SB: "${normalizedPageTitle}" == TMDB: One of "${titlesToCompare
        .filter((t) => t.length > 0)
        .join('", "')}"`
    );
    return true;
  ...
  if (
    titlesToCompare.some(
      (normTmdbTitle) =>
        normTmdbTitle.length > 3 &&
        normalizedPageTitle.length > 3 && // ensure titles aren't too short
        (normalizedPageTitle.includes(normTmdbTitle) ||
          normTmdbTitle.includes(normalizedPageTitle))
    )
  ) {
    console.log(
      `  [Validation] SUCCESS: Partial match. SB: "${normalizedPageTitle}" vs TMDB: One of "${titlesToCompare
        .filter((t) => t.length > 0)
        .join('", "')}"`
    );
    return true;
  ...
  console.log(
    `  [Validation] FAILED: No strong match. SB: "${normalizedPageTitle}" vs TMDB Titles: "${titlesToCompare
      .filter((t) => t.length > 0)
      .join('", "')}"`
  );
  return false;
...
const extractTmdbImagePathFromShowboxHtml = (htmlContent) => {
  ...
  console.log(
    `  [ImageExtract DEBUG] HTML content (first 500 chars): ${String(
      htmlContent
    ).substring(0, 500)}`
  );
  try {
    ...
    if (coverDiv.length) {
      ...
      console.log(
        `  [ImageExtract DEBUG] Found div.cover_follow. Style attribute: ${styleAttr}`
      ); // Log the style attribute
      if (styleAttr) {
        // MODIFIED REGEX: Made &quot; optional
        const urlMatch = styleAttr.match(
          /url\((?:&quot;)?(https?:\/\/image\.tmdb\.org\/t\/p\/original)?([^&]+?)(?:&quot;)?\)/
        );
        if (urlMatch && urlMatch[2]) {
          // Ensure it's a path, not a full URL if accidentally captured
          const imagePath = urlMatch[2].startsWith("/")
            ? urlMatch[2]
            : "/" + urlMatch[2];
          console.log(
            `  [ImageExtract] Extracted TMDB image path from ShowBox HTML: ${imagePath}`
          );
          return imagePath;
        }
      }
    ...
    console.error(
      `  [ImageExtract] Error parsing ShowBox HTML for TMDB image: ${e.message}`
    );
  }
  console.log(
    `  [ImageExtract] Could not find TMDB image path in ShowBox HTML.`
  );
  return null;
};
...
const validateTmdbImage = (showboxImagePath, tmdbApiBackdropPaths = []) => {
  if (
    !showboxImagePath ||
    !tmdbApiBackdropPaths ||
    tmdbApiBackdropPaths.length === 0
  ) {
    return false;
  }
  const match = tmdbApiBackdropPaths.some(
    (apiPath) => apiPath === showboxImagePath
  );
  if (match) {
    console.log(
      `  [ImageValidation] SUCCESS: ShowBox image path "${showboxImagePath}" matches a TMDB API backdrop path.`
    );
  } else {
    console.log(
      `  [ImageValidation] FAILED: ShowBox image path "${showboxImagePath}" does not match any of TMDB API backdrop paths (${tmdbApiBackdropPaths
        .slice(0, 3)
        .join(", ")}...).`
    );
  }
...
const extractSpecialTitles = (tmdbData, alternativeTitles = []) => {
  const specialTitles = [];
  console.log(
    `  [TITLE] Alternative titles available: ${alternativeTitles.length}`
  );
  if (alternativeTitles.length > 0) {
    console.log(
      `  [TITLE] Available alternatives: ${alternativeTitles
        .map(
          (t) =>
            `"${t.title}" (${t.iso_3166_1 || "unknown"}-${
              t.iso_639_1 || "unknown"
            })`
        )
        .join(", ")}`
  ...
  const hasNonLatinChars =
    /[\u0400-\u04FF\u0500-\u052F\u1100-\u11FF\u3000-\u303F\u3040-\u309F\u30A0-\u30FF\u3130-\u318F\u31F0-\u31FF\u3400-\u4DBF\u4E00-\u9FFF\uAC00-\uD7AF\uF900-\uFAFF\uFF00-\uFFEF]/.test(
      originalTitle
    );

  if (hasNonLatinChars) {
    console.log(
      `  [ROMAN] Detected non-Latin title: "${originalTitle}" (Language: ${originalLanguage}). Looking for Romanized version.`
    );
    ...
    const romanizedCandidates = alternativeTitles.filter(
      (alt) =>
    ...
    if (romanizedCandidates.length > 0) {
      romanizedCandidates.forEach((rc) => {
        console.log(
          `  [ROMAN] Found labeled Romanized title: "${rc.title}" (${
            rc.iso_3166_1 || "unknown"
          }-${rc.iso_639_1 || "unknown"}${rc.type ? ", Type: " + rc.type : ""})`
        );
        specialTitles.push(rc.title);
      });
    } else {
      console.log(
        `  [ROMAN] No explicitly labeled Romanized titles found in alternatives.`
      );
      ...
      // 3. Are not English
      const nonEnglishLatinTitles = alternativeTitles.filter(
        (alt) =>
      ...
      const sameRegionTitles = nonEnglishLatinTitles.filter(
        (alt) =>
          alt.iso_3166_1 &&
          (alt.iso_3166_1 === originalLanguage.toUpperCase() ||
            // Special case for languages that don't map directly to country codes
            (originalLanguage === "zh" &&
              ["CN", "TW", "HK"].includes(alt.iso_3166_1)) ||
      ...
      if (sameRegionTitles.length > 0) {
        sameRegionTitles.forEach((title) => {
          console.log(
            `  [ROMAN] Found likely ${languageName} Romanized title: "${title.title}" (matched region)`
          );
        ...
        console.log(
          `  [ROMAN] Using best non-English Latin title as Romanized version: "${bestCandidate.title}"`
        );
        ...
        if (languagePatterns[originalLanguage]) {
          const patternMatches = alternativeTitles.filter(
            (alt) =>
              // Must use Latin script
              /^[a-zA-Z0-9\s\-:;,.!?()&'"]+$/.test(alt.title) &&
          ...
          if (patternMatches.length > 0) {
            patternMatches.forEach((match) => {
              console.log(
                `  [ROMAN] Found likely ${languageName} Romanized title: "${match.title}" (matched language particles)`
              );
              specialTitles.push(match.title);
            });
          } else {
            console.log(
              `  [ROMAN] No titles matching ${languageName} language patterns found.`
            );
            ...
            if (latinTitles.length > 0) {
              ...
              console.log(
                `  [ROMAN] Using longest Latin script title as fallback: "${longestLatin.title}"`
              );
              specialTitles.push(longestLatin.title);
            } else {
              console.log(
                `  [ROMAN] Could not find any suitable Romanized alternative for "${originalTitle}"`
              );
            }
          }
        } else {
          ...
          if (latinTitles.length > 0) {
            ...
            const longestLatin = latinTitles[0];
            console.log(
              `  [ROMAN] Using longest Latin script title for ${languageName}: "${longestLatin.title}"`
            );
            specialTitles.push(longestLatin.title);
          } else {
            console.log(
    ...
    console.log(
      `  [TITLE] Original title already uses Latin script: "${originalTitle}" (${originalLanguage})`
    );

    // Some Latin-script languages might have alternative spellings/titles worth trying
    const nonEnglishAlts = alternativeTitles.filter(
      (alt) =>
    ...
    if (nonEnglishAlts.length > 0) {
      nonEnglishAlts.forEach((alt) => {
        console.log(
          `  [TITLE] Adding non-English alternative title: "${alt.title}"`
        );
...
const _searchAndExtractShowboxUrl = async (
  searchTerm,
  originalTmdbTitle,
  mediaYear,
  ...
  const cacheKeyData = `${CACHE_VERSION}_${tmdbType}_${originalTmdbTitle}_${
    mediaYear || "noYear"
  }`;
  ...
  console.log(
    `  Searching for ShowBox match for: "${originalTmdbTitle}" (${
      mediaYear || "N/A"
    }) [Cache key: ${cacheKeyHash}]`
  );
  ...
  if (process.env.DISABLE_CACHE !== "true") {
    ...
    if (cachedResult) {
      ...
      if (typeof cachedResult === "string") {
        console.log(
          `  CACHE HIT for ShowBox search best match URL (${originalTmdbTitle} ${
            mediaYear || ""
          }): ${cachedResult}`
        );
        ...
        console.log(
          `  CACHE HIT: Previously no match found for ${originalTmdbTitle} ${
            mediaYear || ""
          }, but retrying immediately for fresh content.`
        );
  ...
  const cleanedSearchTerm = searchTerm
    ...
    .trim();
  console.log(
    `  Using cleaned search term: "${cleanedSearchTerm}" (Original: "${searchTerm}")`
  );
  ...
  const specialTitlesFromAll = tmdbAllTitles
    .filter(
      (title) =>
  ...
  if (titleWords.length > 1) {
    ...
    if (
      firstWord.length > 3 &&
      !searchStrategies.some((s) => s.term === firstWord) &&
      mediaYear
    ) {
      searchStrategies.push({
  ...
  searchStrategies.forEach((strategy, idx) => {
    console.log(
      `    ${idx + 1}. ${strategy.description} (Priority: ${
        strategy.priority
      }): "${strategy.term}"`
    );
  });
  ...
  const allPossibleSlugs = tmdbAllTitles
    .map((title) => slugify(title))
    .filter(Boolean);
  console.log(
    `  Generated ${
      allPossibleSlugs.length
    } possible slugs for matching: ${allPossibleSlugs.join(", ")}`
  );
  ...
  for (const strategy of searchStrategies) {
    ...
    if (strategiesAttempted >= MAX_STRATEGIES_TO_TRY) {
      console.log(
        `  Reached maximum number of search strategies (${MAX_STRATEGIES_TO_TRY}). Stopping search.`
      );
      break;
    }

    strategiesAttempted++;
    const searchUrl = `https://www.showbox.media/search?keyword=${encodeURIComponent(
      strategy.term
    )}`;
    console.log(
      `  Searching ShowBox with URL: ${searchUrl} (Strategy: ${strategy.description})`
    );

    const htmlContent = await showboxScraperInstance._makeRequest(searchUrl);
    if (!htmlContent) {
      console.log(
        `  Failed to fetch ShowBox search results for strategy: ${strategy.description}`
      );
    ...
    const normalizedTmdbTitles = tmdbAllTitles.map((title) =>
    ...
    $("div.film-poster").each((i, elem) => {
      ...
      if (itemTitle && itemHref) {
        ...
        const urlSlugMatch = itemHref.match(
          /\/(movie|tv)\/([mt]-[a-z0-9-]+-\d{4})/
        );
        const itemSlug = urlSlugMatch ? urlSlugMatch[2] : "";

        // Check if the slug matches any of our generated slugs
        const slugMatchScore = allPossibleSlugs.some(
          (slug) =>
            itemSlug.includes(slug) || // Direct inclusion of our slug in their slug
            allPossibleSlugs.some((ourSlug) =>
              ourSlug.includes(itemSlug.replace(/^[mt]-/, ""))
            ) // Their slug in our slug
        )
          ? 10
          : 0;
        ...
        const correctMediaType =
          (tmdbType === "movie" && itemHref.includes("/movie/")) ||
        ...
        if (
          normalizedTmdbTitles.some(
            (normTitle) => normTitle === normalizedItemTitle
          )
        ) {
        ...
        // Title contains our title or vice versa (any of our titles)
          ...
          normalizedTmdbTitles.some(
            (normTitle) =>
              normTitle.length > 3 &&
              normalizedItemTitle.length > 3 &&
              (normalizedItemTitle.includes(normTitle) ||
                normTitle.includes(normalizedItemTitle))
          )
        ) {
        ...
        // TMDB title contains the simplified item title (could be abbreviation/shortened)
          ...
          simplifiedTmdbTitle.includes(simplifiedItemTitle) &&
          simplifiedItemTitle.length > 3
        ) {
          ...
          if (
            itemWords.some((itemWord) => {
              ...
              if (tmdbWord.length > 3 && itemWord.length > 3) {
                return (
                  (itemWord.includes(tmdbWord) &&
                    itemWord.length - tmdbWord.length <= 2) ||
                  (tmdbWord.includes(itemWord) &&
                    tmdbWord.length - itemWord.length <= 2)
                );
              }
              return false;
            })
    ...
    if (searchResults.length > 0) {
      ...
      searchResults.slice(0, 3).forEach((result, i) => {
        console.log(
          `    ${i + 1}. Title: "${result.title}", Year: ${
            result.year || "N/A"
          }, Score: ${result.score.toFixed(1)}, URL: ${
            result.href
          }, Media Type: ${
            result.isMovie ? "Movie" : result.isTv ? "TV" : "Unknown"
          }`
        );
      });
      ...
      if (bestMatch.score > bestResult.score) {
        bestResult = {
          ...
          year: bestMatch.year,
          isCorrectType:
            (tmdbType === "movie" && bestMatch.isMovie) ||
            (tmdbType === "tv" && bestMatch.isTv),
        };
    ...
    if (bestResult.score > 35 && bestResult.isCorrectType) {
      console.log(
        `  Found good match with score ${bestResult.score.toFixed(
          1
        )} using strategy "${bestResult.strategy}", stopping search`
      );
      break;
    }
  }
  ...
  Object.entries(strategyResults).forEach(([strategy, result]) => {
    if (result.success) {
      const status = result.notBest
        ? "Found results but not best"
        : result.correctType
        ? "Found correct type"
        : "Found wrong type";
      console.log(
        `    - ${strategy}: ${status} (Score: ${result.score.toFixed(1)})`
      );
    } else {
  ...
  if (bestResult.url) {
    ...
    if (bestResult.score >= 40 && bestResult.isCorrectType) {
      ...
      console.log(
        `  Low confidence match (Score: ${bestResult.score.toFixed(
          1
        )}). Discarding - requires score >= 40.`
      );
      useResult = false;
    }
    ...
    if (useResult && bestResult.url) {
      console.log(
        `  Best overall match: ${
          bestResult.url
        } (Score: ${bestResult.score.toFixed(1)}, Strategy: ${
          bestResult.strategy
        })`
      );
  ...
  console.log(
    `  No suitable match found on ShowBox search for: ${originalTmdbTitle} (${
      mediaYear || "N/A"
    })`
  );
  if (process.env.DISABLE_CACHE !== "true") {
    await saveToCache(
      searchTermKey,
      { result: "NO_MATCH_FOUND", timestamp: Date.now() },
      cacheSubDir
    );
  }
...
const getShowboxUrlFromTmdbInfo = async (
  tmdbType,
  tmdbId,
  regionPreference = null
) => {
  ...
  if (!disableCache) {
    ...
    if (cachedResult) {
      if (cachedResult.showboxUrl === "NO_URL_FOUND") {
        // For stream searches, always retry immediately - content availability can change anytime
        console.log(
          `  [Final URL Cache] HIT: Previously no URL found for ${tmdbType}/${tmdbId}, but retrying immediately for fresh content.`
        );
        // Continue to search logic below to retry immediately
      } else {
        console.log(
          `  [Final URL Cache] HIT: Found cached ShowBox result for ${tmdbType}/${tmdbId}.`
        );
        return cachedResult;
      }
    }
  ...
  if (!tmdbData) {
    console.log(
      `  Could not fetch TMDB data for ${tmdbType}/${tmdbId}. Cannot proceed.`
    );
    console.timeEnd("getShowboxUrlFromTmdbInfo_total");
    return null;
  }
  ...
  let tmdbAlternativeTitlesData = await getFromCache(
    altTitlesCacheKey,
    mainCacheSubDir
  );
  ...
  if (tmdbData.genres && Array.isArray(tmdbData.genres)) {
    ...
    if (animeGenre) {
      console.log(
        `  [ANIME] Content identified as Animation genre, will check for anime-specific titles`
      );
    }
  }
  ...
  if (!mainTitle && !originalTitle) {
    console.log(
      `  Could not determine any title from TMDB data for ${tmdbType}/${tmdbId}.`
    );
    console.timeEnd("getShowboxUrlFromTmdbInfo_total");
    return null;
  }
  ...
  const allTitles = [
    ...
    originalTitle,
    ...specialTitles,
    ...alternativeTitles.map((alt) => alt.title),
  ]
    .filter(Boolean) // Remove nulls/undefined
    .filter((value, index, self) => self.indexOf(value) === index); // Remove duplicates

  console.log(
    `  Collected ${allTitles.length} unique titles for "${
  ...
  console.log(
    `  Limiting direct URL construction attempts to first ${titlesForDirectAttempt.length} titles out of ${allTitles.length} total unique titles.`
  );
  ...
  const findUrl = async () => {
    ...
    if (year) {
      console.log(
        `  Attempting direct ShowBox URL construction for ${tmdbType} "${mainTitle}" (${year}) with ${titlesForDirectAttempt.length} title variants.`
      );
      ...
      for (const candidateTitle of titlesForDirectAttempt) {
        ...
        console.log(
          `    Trying direct URL: ${directShowboxUrl} (from title: "${candidateTitle}")`
        );

        const htmlContent = await showboxScraperInstance._makeRequest(
          directShowboxUrl
        );
        if (htmlContent) {
          console.log(
            `    Successfully fetched content from direct URL: ${directShowboxUrl}`
          );
          const pageInfo = showboxScraperInstance.extractContentIdAndType(
            directShowboxUrl,
            htmlContent
          );

          if (pageInfo && pageInfo.title) {
            const titleIsValid = validateShowboxTitle(
              pageInfo.title,
              mainTitle,
              originalTitle,
              alternativeTitles
            );
            const showboxTmdbImagePath =
              extractTmdbImagePathFromShowboxHtml(htmlContent);
            const imageIsValid = validateTmdbImage(
              showboxTmdbImagePath,
              tmdbBackdropPaths
            );

            if (titleIsValid && imageIsValid) {
              console.log(
                `      SUCCESS (TITLE & IMAGE VALIDATED): Validated title and TMDB image for ${directShowboxUrl}. Using this URL.`
              );
              return {
                showboxUrl: directShowboxUrl,
                year: year,
                title: mainTitle,
              };
            }
    ...
    console.log(
      `  No validated ShowBox URL found through direct construction. Falling back to search...`
    );
    const searchResult = await _searchAndExtractShowboxUrl(
      mainTitle,
      mainTitle,
      year,
      showboxScraperInstance,
    ...
    if (searchResult && searchResult.url) {
      console.log(
        `  Search found a result. Title: ${searchResult.title}, Year: ${searchResult.year}`
      );
  ...
  if (!disableCache) {
    if (finalResult && finalResult.showboxUrl) {
      console.log(
        `  [Final URL Cache] SAVING result for ${tmdbType}/${tmdbId}: ${finalResult.showboxUrl}`
      );
      await saveToCache(urlCacheKey, finalResult, urlCacheSubDir);
    } else {
      console.log(
        `  [Final URL Cache] SAVING 'NO_URL_FOUND' for ${tmdbType}/${tmdbId}.`
      );
      await saveToCache(
        urlCacheKey,
        { showboxUrl: "NO_URL_FOUND", timestamp: Date.now() },
        urlCacheSubDir
      );
    }
  }
...
const fetchSourcesForSingleFid = async (
  fidToProcess,
  shareKey,
  regionPreference = null,
  userCookie = null,
  ...
  const cookieForRequest = await getCookieForRequest(
    regionPreference,
    userCookie
  );
  ...
  console.log(
    `  Fetching fresh player data for video FID: ${fidToProcess} (Share: ${shareKey}) via ${
      finalPostUrl === FEBBOX_PLAYER_URL ? "direct" : "proxy"
    } to ${FEBBOX_PLAYER_URL}`
  );
  ...
  try {
    ...
    if (!sourcesMatch) {
      console.log(
        `    Could not find sources array in player response for FID ${fidToProcess}`
      );
      if (
        playerContent.startsWith("http") &&
        (playerContent.includes(".mp4") || playerContent.includes(".m3u8"))
      ) {
      ...
      try {
        ...
        if (jsonResponse.msg) {
          console.log(
            `    FebBox API Error: ${jsonResponse.code} - ${jsonResponse.msg}`
          );
          // Check for region-specific errors
          if (
            jsonResponse.code === 1002 ||
            ...
            if (retryAttempt < MAX_RETRIES_SAME_REGION) {
              console.log(
                `    Region error on attempt ${
                  retryAttempt + 1
                }. Retrying same region: ${global.lastRequestedRegion.used}`
              );
              await new Promise((resolve) => setTimeout(resolve, 1000)); // Wait 1 second
              return fetchSourcesForSingleFid(
                fidToProcess,
                shareKey,
                regionPreference,
                userCookie,
            ...
            if (global.lastRequestedRegion && global.lastRequestedRegion.used) {
              console.log(
                `    Marking region ${
                  global.lastRequestedRegion.used
                } as unavailable after ${retryAttempt + 1} failed attempts.`
              );
              global.regionAvailabilityStatus[
                global.lastRequestedRegion.used
              ] = false;
              ...
              if (!global.lastRequestedRegion.usingFallback) {
                for (const fallbackRegion of US_FALLBACK_REGIONS) {
                  if (fallbackRegion !== global.lastRequestedRegion.used) {
                    console.log(
                      `    Retrying with fallback US region: ${fallbackRegion}`
                    );
      ...
      if (sourceItem.file && sourceItem.label) {
        ...
        try {
          const urlParams = new URLSearchParams(
            new URL(sourceItem.file).search
          );
    ...
    if (fidVideoLinks.length > 0) {
      console.log(
        `    Extracted ${fidVideoLinks.length} fresh video link(s) for FID ${fidToProcess}`
      );
    }
    ...
    if (
      ...
      if (global.lastRequestedRegion && global.lastRequestedRegion.used) {
        console.log(
          `    Marking region ${
            global.lastRequestedRegion.used
          } as potentially unavailable after ${
            retryAttempt + 1
          } failed attempts due to error.`
        );
        global.regionAvailabilityStatus[
          global.lastRequestedRegion.used
        ] = false;
...
class ShowBoxScraper {
  ...
  getNextProxy() {
    ...
    if (this.userScraperApiKey && this.proxyCounter % 10 < 3) {
      console.log(
        `[Rotating Proxy] Using ScraperAPI for request #${this.proxyCounter} (user-provided key)`
      );
      // Return a function that will format the URL for ScraperAPI
      return (url) => {
        const scraperApiUrl = "https://api.scraperapi.com/";
        return `${scraperApiUrl}?api_key=${
          this.userScraperApiKey
        }&url=${encodeURIComponent(url)}&country_code=us`;
      };
    }
    ...
    console.log(
      `[Rotating Proxy] Selected proxy ${proxyIndex + 1}/2 for request #${
        this.proxyCounter
      }`
    );
  ...
  async _makeRequest(url, isJsonExpected = false) {
    const timerLabel = `ShowBoxScraper_makeRequest_${crypto
      .createHash("md5")
      .update(url)
      .digest("hex")}`;
    ...
    if (selectedProxy) {
      if (typeof selectedProxy === "function") {
        ...
        console.log(
          `ShowBoxScraper: Making request to: ${url} via Proxy: ${selectedProxy}`
        );
      }
    } else {
      console.log(
        `ShowBoxScraper: Making direct request to: ${url} (no proxy available)`
      );
    }
    ...
    if (url.includes("febbox.com") && !isUsingScraperApi) {
      if (this.regionPreference) {
        console.log(
          `[ShowBoxScraper] Getting cookie with explicit region preference: ${this.regionPreference}`
        );
        cookieValue = await getCookieForRequest(
          this.regionPreference,
          this.userCookie
        );
      } else {
        console.log(
          `[ShowBoxScraper] Getting cookie with default region (no preference specified)`
        );
  ...
  extractContentIdAndType(url, htmlContent) {
    ...
    const urlMatchDetail = url.match(
      /\/(movie|tv)\/detail\/(\d+)(?:-([a-zA-Z0-9-]+))?/
    );
    if (urlMatchDetail) {
    ...
    if (htmlContent) {
      ...
      if (extractedHtmlTitle) {
        ...
        if (title.includes(" - ShowBox"))
          title = title.split(" - ShowBox")[0].trim();
        if (title.includes("| ShowBox"))
          title = title.split("| ShowBox")[0].trim();
      }

      if (!contentId || !contentTypeVal) {
        const headingLinkSelector =
          'h2.heading-name a[href*="/detail/"], h1.heading-name a[href*="/detail/"]';
        ...
        if (!contentId) {
          ...
          if (shareDiv.length) {
            let linkElements = shareDiv.find(
              'a[href*="/movie/detail/"], a[href*="/tv/detail/"]'
            );
            const dataUrlOnDiv = shareDiv.attr("data-url");
            if (linkElements.length === 0 && dataUrlOnDiv) {
              const dummyLinkSoup = cheerio.load(
                `<a href="${dataUrlOnDiv}"></a>`
              );
            ...
            linkElements.each((i, el) => {
              ...
              if (href) {
                const idTypeMatchShare = href.match(
                  /\/(movie|tv)\/detail\/(\d+)/
                );
                if (idTypeMatchShare) {
  ...
  async extractFebboxShareLinks(showboxUrl) {
    const timerLabel = `extractFebboxShareLinks_total_${showboxUrl.replace(
      /[^a-zA-Z0-9]/g,
      ""
    )}`;
    console.log(
      `ShowBoxScraper: Attempting to extract FebBox share link from: ${showboxUrl}`
    );
    console.time(timerLabel);
    ...
    try {
      ...
      if (!contentInfo || !contentInfo.id || !contentInfo.type) {
        ...
        if (!htmlContent) {
          console.log(
            `ShowBoxScraper: Failed to fetch HTML for ${showboxUrl}.`
          );
          console.timeEnd(timerLabel);
          return [];
        }
      ...
      if (!contentInfo || !contentInfo.id || !contentInfo.type) {
        console.log(
          `ShowBoxScraper: Could not determine content ID/type for ${showboxUrl}.`
        );
        console.timeEnd(timerLabel);
        return [];
      }
      ...
      if (htmlContent) {
        ...
        if (!directFebboxLink) {
          const scriptContents = $("script")
            ...
            .get()
            .join("\n");
          const shareKeyMatch = scriptContents.match(
            /['"](https?:\/\/www\.febbox\.com\/share\/[a-zA-Z0-9-]+)['"]/
          );
        ...
        if (directFebboxLink) {
          console.log(
            `ShowBoxScraper: Successfully fetched FebBox URL: ${directFebboxLink} from HTML`
          );
          console.timeEnd(timerLabel);
          return [
            {
      ...
      console.log(
        `ShowBoxScraper: Making API call for ${title} (ID: ${contentId}, Type: ${contentType})`
      );
      ...
      if (!apiResponseStr) {
        console.log(
          `ShowBoxScraper: Failed to get response from ShowBox share_link API`
        );
        console.timeEnd(timerLabel);
        return [];
      }
      ...
      try {
        const apiResponseJson =
          typeof apiResponseStr === "string"
            ? JSON.parse(apiResponseStr)
            : apiResponseStr;
        if (
          apiResponseJson.code === 1 &&
          apiResponseJson.data &&
          apiResponseJson.data.link
        ) {
          const febboxShareUrl = apiResponseJson.data.link;
          console.log(
            `ShowBoxScraper: Successfully fetched FebBox URL: ${febboxShareUrl} from API`
          );
          console.timeEnd(timerLabel);
          return [
            {
          ...
          console.log(
            `ShowBoxScraper: ShowBox share_link API did not succeed for '${title}'.`
          );
          console.timeEnd(timerLabel);
          return [];
        }
        ...
        console.log(
          `ShowBoxScraper: Error decoding JSON from ShowBox share_link API: ${e.message}`
        );
        console.timeEnd(timerLabel);
        return [];
      }
      ...
      console.error(
        `ShowBoxScraper: Unexpected error in extractFebboxShareLinks for ${showboxUrl}: ${error.message}`
      );
      console.timeEnd(timerLabel);
      return [];
    }
...
const extractFidsFromFebboxPage = async (
  febboxUrl,
  regionPreference = null,
  userCookie = null
) => {
  const timerLabel = `extractFidsFromFebboxPage_total_${febboxUrl.replace(
    /[^a-zA-Z0-9]/g,
    ""
  )}`;
  // console.time(timerLabel);
  ...
  const cachedParsedData = await getFromCache(
    cacheKeyParsed,
    cacheSubDirParsed
  );
  if (cachedParsedData && typeof cachedParsedData === "object") {
  ...
  const cookieForRequest = await getCookieForRequest(
    regionPreference,
    userCookie
  ); // Pass region preference and user cookie
  const baseHeaders = { Cookie: `ui=${cookieForRequest}` };
  const fetchTimerLabel = `extractFidsFromFebboxPage_fetch_${febboxUrl.replace(
    /[^a-zA-Z0-9]/g,
    ""
  )}`;
  ...
  // }
  console.log(
    `Fetching FebBox page content from URL: ${febboxUrl} via ${
      finalGetUrl === febboxUrl ? "direct" : "proxy"
    }`
  );
  ...
  if (matchShareKeyUrl) {
    ...
    // Update regex to include hyphens in share key pattern
    const matchShareKeyHtml = contentHtml.match(
      /(?:var share_key\s*=|share_key:\s*|shareid=)"?([a-zA-Z0-9-]+)"?/
    );
  ...
  const playerSetupMatch = contentHtml.match(
    /jwplayer\("[a-zA-Z0-9_-]+"\)\.setup/
  );
  if (playerSetupMatch) {
  ...
  if (
    ...
    !(directSources && directSources.length > 0)
  ) {
    // Check if directSources also not found
    console.log(
      `No files or direct sources found on FebBox page: ${febboxUrl}`
    );
    // console.timeEnd(timerLabel);
...
const convertImdbToTmdb = async (
  ...
  console.log(
    `  Attempting to convert IMDb ID: ${imdbId} to TMDB ID${
  ...
  if (cachedData) {
    console.log(
      `    IMDb to TMDB conversion found in CACHE for ${imdbId}:`,
      cachedData
    );
    ...
    console.log(
      "    Cached data for IMDb conversion is malformed. Fetching fresh."
    );
  }
    ...
    if (findResults) {
      ...
      if (expectedType === "tv" || expectedType === "series") {
        ...
        if (findResults.tv_results && findResults.tv_results.length > 0) {
          result = {
            tmdbId: String(findResults.tv_results[0].id),
            tmdbType: "tv",
            title:
              findResults.tv_results[0].name ||
              findResults.tv_results[0].original_name,
          };
          console.log(
            `    Prioritized TV result for series request: ${result.title}`
          );
        } else if (
          findResults.movie_results &&
          findResults.movie_results.length > 0
        ) {
          result = {
            tmdbId: String(findResults.movie_results[0].id),
            tmdbType: "movie",
            title:
              findResults.movie_results[0].title ||
              findResults.movie_results[0].original_title,
          };
          console.log(
            `    Fallback to movie result for series request: ${result.title}`
          );
        }
        ...
        if (findResults.movie_results && findResults.movie_results.length > 0) {
          ...
          console.log(
            `    Prioritized movie result for movie request: ${result.title}`
          );
        } else if (
          findResults.tv_results &&
          findResults.tv_results.length > 0
        ) {
          result = {
            tmdbId: String(findResults.tv_results[0].id),
            tmdbType: "tv",
          ...
          console.log(
            `    Fallback to TV result for movie request: ${result.title}`
          );
        }
      } else {
      ...
      if (
        findResults.person_results &&
        findResults.person_results.length > 0 &&
        !result
      ) {
        // Could handle other types if necessary, e.g. person, but for streams, movie/tv are key
        console.log(
          `    IMDb ID ${imdbId} resolved to a person, not a movie or TV show on TMDB.`
        );
      } else if (!result) {
        console.log(
          `    No movie or TV results found on TMDB for IMDb ID ${imdbId}. Response:`,
          JSON.stringify(findResults).substring(0, 200)
        );
      }

      if (result && result.tmdbId && result.tmdbType) {
        console.log(
          `    Successfully converted IMDb ID ${imdbId} to TMDB ${result.tmdbType} ID ${result.tmdbId} (${result.title})`
        );
        ...
        console.log(
          `    Could not convert IMDb ID ${imdbId} to a usable TMDB movie/tv ID.`
        );
      }
    }
  } catch (error) {
    if (console.timeEnd && typeof console.timeEnd === "function")
      console.timeEnd(`convertImdbToTmdb_apiCall_${imdbId}`); // Ensure timer ends on error
    const errorMessage = error.response
      ? `${error.message} (Status: ${error.response.status})`
      : error.message;
    console.log(
      `    Error during TMDB find API call for IMDb ID ${imdbId}: ${errorMessage}`
    );
  }
  console.timeEnd(`convertImdbToTmdb_total_${imdbId}`);
  return null;
...
const getStreamsFromTmdbId = async (
  ...
  const mainTimerLabel =
    `getStreamsFromTmdbId_total_${tmdbType}_${tmdbId}` +
    (seasonNum ? `_s${seasonNum}` : "") +
    (episodeNum ? `_e${episodeNum}` : "");
  console.time(mainTimerLabel);
  console.log(
    `Getting streams for TMDB ${tmdbType}/${tmdbId}${
  ...
  if (ENABLE_PSTREAM_API) {
    try {
      ...
      if (tmdbData && tmdbData.imdbId) {
        console.log(
          `[PStream Primary] Attempting PStream API for IMDB ID: ${tmdbData.imdbId}`
        );
        ...
        if (pstreamStreams && pstreamStreams.length > 0) {
          console.log(
            `[PStream Primary] SUCCESS: Found ${pstreamStreams.length} streams from PStream API`
          );
          ...
          console.log(
            `[PStream Primary] No streams found from PStream API, falling back to FebBox`
          );
        }
      } else {
        console.log(
          `[PStream Primary] Could not get IMDB ID from TMDB data, falling back to FebBox`
        );
      }
    } catch (error) {
      console.error(
        `[PStream Primary] Error with PStream API: ${error.message}, falling back to FebBox`
      );
    }
  } else {
    console.log(
      "[PStream] PStream API is disabled, skipping to FebBox fallback"
    );
  }

  // FALLBACK METHOD: Use original FebBox/ShowBox logic
  console.log(`[Fallback] Using FebBox/ShowBox method`);
  const tmdbInfo = await getShowboxUrlFromTmdbInfo(
    tmdbType,
    tmdbId,
    regionPreference
  );
  ...
  if (!tmdbInfo || !tmdbInfo.showboxUrl) {
    console.log(
      `Could not construct ShowBox URL for TMDB ${tmdbType}/${tmdbId}`
    );
    console.timeEnd(mainTimerLabel);
    return [];
  }
  ...
  if (!febboxShareInfos) {
    ...
    console.log(
      `No cached FebBox links found, extracting from ShowBox: ${showboxUrl}`
    );
    ...
    if (process.env.DISABLE_CACHE !== "true") {
      ...
      console.log(
        `[Final URL Cache] UPDATED with FebBox links for ${tmdbType}/${tmdbId}`
      );
    }
  } else {
    console.log(
      `[Final URL Cache] Using cached FebBox links, skipping ShowBox request for ${showboxUrl}`
    );
  }
  ...
  const streamPromises = febboxShareInfos.map(async (shareInfo) => {
    ...
    try {
      ...
      if (tmdbType === "tv" && seasonNum !== null) {
        ...
        const tvStreams = await processShowWithSeasonsEpisodes(
          febboxUrl,
          baseStreamTitle,
          seasonNum,
          episodeNum,
        ...
        if (directSources && directSources.length > 0) {
          ...
          // Only process FIDs if no directSources were found
      ...
      console.error(
        `Error processing share info for ${shareInfo.febbox_share_url}: ${error.message}`
      );
    }
  ...
  if (allStreams.length > 0) {
    ...
    const streamsWithSizes = await Promise.all(sizePromises);
    console.timeEnd(
      `getStreamsFromTmdbId_fetchStreamSizes_${tmdbType}_${tmdbId}`
    );
  }
  ...
  const streamsToShowBoxFiltered = sortedStreams.filter((stream) => {
    ...
    if (quality === "360p" || quality === "480p") {
      console.log(
        `  Filtering out potential ShowBox low-quality stream: ${stream.title} (${stream.quality})`
      );
      return false;
    }
    return true;
  ...
  if (!userCookie && !global.currentRequestUserCookie) {
    console.log(
      "[SizeLimit] No personal cookie detected. Applying 9GB size limit to ShowBox streams."
    );
    ...
    finalFilteredStreams = streamsToShowBoxFiltered.filter((stream) => {
      ...
      if (sizeInBytes >= NINE_GB_IN_BYTES) {
        console.log(
          `[SizeLimit] Filtering out ShowBox stream due to size (${
  ...
  if (finalFilteredStreams.length > 0) {
    console.log(
      `Found ${finalFilteredStreams.length} streams (sorted, ShowBox-low-quality-filtered, and size-limited if applicable):`
    );
    finalFilteredStreams.slice(0, 5).forEach((stream, i) => {
      console.log(
        `  ${i + 1}. ${stream.quality} (${stream.size || "Unknown size"}) [${
          (stream.codecs || []).join(", ") || "No codec info"
        }]: ${stream.title}`
      );
    });
...
const processShowWithSeasonsEpisodes = async (
  febboxUrl,
  showboxTitle,
  seasonNum,
  episodeNum,
  ...
  const processTimerLabel =
    `processShowWithSeasonsEpisodes_total_s${seasonNum}` +
    (episodeNum ? `_e${episodeNum}` : "_all") +
    (resolveFids ? "_resolve" : "_noresolve") +
    (retryAttempt > 0 ? `_retry${retryAttempt}` : "");
  console.time(processTimerLabel);
  console.log(
    `Processing TV Show: ${showboxTitle}, Season: ${seasonNum}, Episode: ${
      episodeNum !== null ? episodeNum : "all"
    }${resolveFids ? "" : " (FIDs not resolved)"}${
  ...
  let contentHtml =
    retryAttempt > 0 ? null : await getFromCache(cacheKeyMain, cacheSubDirMain);
  ...
  if (!contentHtml) {
    ...
    console.log(
      `Fetching main FebBox page ${febboxUrl} directly${
        retryAttempt > 0 ? " (forced refresh for missing season)" : ""
      }`
    );
    ...
    try {
      ...
      console.log(
        `Failed to fetch HTML content from ${febboxUrl}: ${error.message}`
      );
      console.timeEnd(fetchMainPageTimer);
  ...
  const $ = cheerio.load(contentHtml);
  const shareKey = contentHtml.match(
    /(?:var share_key\s*=|share_key:\s*|shareid=)"?([a-zA-Z0-9-]+)"?/
  )?.[1];
  ...
  fileElements.each((index, element) => {
    ...
    const folderName = folderNameEl.length
      ? folderNameEl.text().trim()
      : feEl.attr("data-path") || `Folder_${dataId}`;
    ...
    if (extractedSeasonNum === null) {
      ...
      if (numMatches && numMatches.length === 1) {
        // Only if exactly one number is found
  ...
  if (folders.length === 0) {
    ...
    const { fids, directSources } = await extractFidsFromFebboxPage(
      febboxUrl,
      regionPreference,
      userCookie
    );
    console.timeEnd(
      `processShowWithSeasonsEpisodes_fallbackDirect_s${seasonNum}`
    );

    if (directSources && directSources.length > 0) {
      for (const source of directSources) {
        const streamTitle = `${showboxTitle} - ${source.label}`;
        streamsForThisCall.push({
          // MODIFIED: Push to streamsForThisCall
          title: streamTitle,
          url: source.url,
          quality: parseQualityFromLabel(source.label),
    ...
    if (fids.length > 0) {
      console.time(
        `processShowWithSeasonsEpisodes_fallbackFids_s${seasonNum}_concurrent`
      );
      const fallbackFidPromises = fids.map((fid) =>
  ...
  console.log(
    `Found ${folders.length} season folders:`,
    folders
      .map(
        (f) =>
          `"${f.name}" (ID: ${f.id}, Extracted Season: ${
            f.extractedSeasonNum !== null ? f.extractedSeasonNum : "None"
          })`
      )
      .join(", ")
  );
  ...
  for (const folder of folders) {
    if (folder.extractedSeasonNum === seasonNum) {
      ...
      console.log(
        `Found exact season match: "${folder.name}" with extracted season number ${folder.extractedSeasonNum}`
      );
      break;
    }
  }
  ...
  if (!selectedFolder && seasonNum > 0 && seasonNum <= folders.length) {
    ...
    console.log(
      `Sorted folders by season number:`,
      sortedFolders
        .map(
          (f) =>
            `"${f.name}" (ID: ${f.id}, Extracted Season: ${
    ...
    const exactExtractedMatch = sortedFolders.find(
      (f) => f.extractedSeasonNum === seasonNum
    );
    if (exactExtractedMatch) {
      selectedFolder = exactExtractedMatch;
      console.log(
        `Found exact season match in sorted folders: "${selectedFolder.name}" with season ${selectedFolder.extractedSeasonNum}`
      );
    }
    // Otherwise, if we have folders with extracted season numbers, use them for mapping
      ...
      const validFolders = sortedFolders.filter(
        (f) => f.extractedSeasonNum !== null
      );
      if (seasonNum <= validFolders.length) {
        selectedFolder = validFolders[seasonNum - 1];
        console.log(
          `Using sorted folder by extracted number index: "${selectedFolder.name}" at position ${seasonNum}`
        );
      }
    }
    // Last resort: use index-based approach on original folder order
      ...
      console.log(
        `Using original folder order index: "${selectedFolder.name}" at position ${seasonNum}`
      );
    }
  }
  ...
  if (!selectedFolder) {
    ...
    if (retryAttempt === 0) {
      console.log(
        `Season ${seasonNum} not found in cached data. Attempting fresh fetch...`
      );
      console.timeEnd(processTimerLabel);
      // Recursive call with retry attempt to force fresh fetch
      return await processShowWithSeasonsEpisodes(
        febboxUrl,
        showboxTitle,
        seasonNum,
        episodeNum,
      ...
      console.log(
        `Could not find season ${seasonNum} folder in ${febboxUrl} even after fresh fetch. Aborting.`
      );
      console.timeEnd(processTimerLabel);
      return streamsForThisCall;
    }
  ...
  console.log(
    `Selected season folder: ${selectedFolder.name} (ID: ${selectedFolder.id})`
  );
  ...
  const cachedParsedEpisodeList = await getFromCache(
    cacheKeyFolderParsed,
    cacheSubDirFolderParsed
  );
  if (cachedParsedEpisodeList && Array.isArray(cachedParsedEpisodeList)) {
    console.log(
      `  CACHE HIT for parsed episode list: Season ${seasonNum}, Folder ${selectedFolder.id}`
    );
    ...
    if (!folderHtml) {
      ...
      try {
        ...
        const cookieForRequestFolder = await getCookieForRequest(
          regionPreference,
          userCookie
        ); // Simplified cookie call
        ...
        console.log(
          `Fetching FebBox folder ${targetFolderListUrl} via ${
            finalFolderUrl === targetFolderListUrl ? "direct" : "proxy"
          }`
        );
        ...
        console.log(
          `  FebBox folder list response status: ${folderResponse.status}`
        );
        console.log(
          `  FebBox folder list response content-type: ${folderResponse.headers["content-type"]}`
        );
        // Log the beginning of the data to inspect its structure
        const responseDataPreview =
          typeof folderResponse.data === "string"
            ? folderResponse.data.substring(0, 500)
            : JSON.stringify(folderResponse.data).substring(0, 500);
        console.log(
          `  FebBox folder list response data (preview): ${responseDataPreview}`
        );

        if (
          folderResponse.data &&
          typeof folderResponse.data === "object" &&
          folderResponse.data.html
        ) {
          folderHtml = folderResponse.data.html;
          console.log(
            `    Successfully extracted HTML from FebBox folder list JSON response.`
          );
        } else if (typeof folderResponse.data === "string") {
          folderHtml = folderResponse.data;
          console.log(
            `    Received direct HTML string from FebBox folder list response.`
          );
        } else {
          console.log(
            `    Invalid or unexpected response format from FebBox folder API for ${
              selectedFolder.id
            }. Data: ${JSON.stringify(folderResponse.data)}`
          );
          // folderHtml remains null
        }

        if (folderHtml && folderHtml.trim().length > 0) {
          // Also check if html is not just whitespace
          await saveToCache(
            cacheKeyFolderHtml,
            folderHtml,
            cacheSubDirFolderHtml
          );
          console.log(`    Cached FebBox folder HTML for ${selectedFolder.id}`);
        } else {
          console.log(
            `    Folder HTML from FebBox was empty or null for ${selectedFolder.id}. Not caching.`
          );
        ...
        console.error(
          `  ERROR fetching FebBox folder content for ${selectedFolder.id}: ${error.message}`
        );
        if (error.response) {
          console.error(`    FebBox Error Status: ${error.response.status}`);
          console.error(
            `    FebBox Error Data: ${JSON.stringify(error.response.data)}`
          );
        }
  ...
  if (episodeDetails.length === 0) {
    // Only proceed if we didn't get data from parsed cache
    if (!folderHtml) {
      // If still no folderHtml (e.g. HTML cache miss and fetch fail), then error out
      console.log(
        `No folder HTML content available (and no cached parsed list) for folder ${selectedFolder.id}`
      );
      console.timeEnd(processTimerLabel);
      return streamsForThisCall;
    }
    ...
    $folder("div.file").each((index, element) => {
      ...
      const fileName = fileNameEl.length
        ? fileNameEl.text().trim()
        : `File_${dataId}`;
  ...
  if (episodeNum !== null) {
    ...
    if (
      !selectedEpisode &&
      episodeNum > 0 &&
      episodeNum <= episodeDetails.length
    ) {
    ...
    if (!selectedEpisode) {
      console.log(
        `Could not find episode ${episodeNum} in season folder ${selectedFolder.name}`
      );
      console.timeEnd(processTimerLabel);
      return streamsForThisCall;
    }
    ...
    console.log(
      `Found episode: ${selectedEpisode.name} (FID: ${selectedEpisode.fid})`
    );
    ...
    if (episodeDetails.length > 0) {
      ...
      console.log(
        `  No episode details found for folder ${selectedFolder.name} to extract FIDs for all episodes.`
      );
    }
  }

  // Get video sources for each episode FID
  if (resolveFids && episodeFids.length > 0) {
    // Check resolveFids flag here
    const episodeTimerLabel =
      `processShowWithSeasonsEpisodes_fetchEpisodeSources_s${seasonNum}` +
      (episodeNum ? `_e${episodeNum}` : "_allEp_concurrent");
    ...
    for (const result of episodeSourcesResults) {
      ...
      if (
        ...
        for (const source of sources) {
          ...
          const streamTitle = `${showboxTitle} - S${seasonNum}${
            episodeNum && selectedEpisode && fid === selectedEpisode.fid
              ? `E${episodeNum}`
              : episodeDetail
              ? `E${episodeDetail.episodeNum}`
              : ""
          } - ${episodeName} - ${source.label}`;
          streamsForThisCall.push({
            // MODIFIED: Push to streamsForThisCall
            title: streamTitle,
            url: source.url,
        ...
        console.log(
          `  Warning: Invalid result structure for an episode source: ${JSON.stringify(
            result
          )}`
        );
      }
    ...
    console.log(
      `  Skipping FID resolution for ${episodeFids.length} episodes in S${seasonNum} as per request.`
    );
  }
  console.timeEnd(processTimerLabel);
...
const getEpisodeNumberFromName = (name) => {
  ...
  const simpleNumMatches = nameLower.match(
    /(?<![a-zA-Z])(\d{1,3})(?![a-zA-Z0-9])/g
  );
  if (simpleNumMatches && simpleNumMatches.length === 1) {
    ...
    if (
      ...
      (nameLower.split(simpleNumMatches[0]).length - 1 <= 1 ||
        !["1", "2"].includes(simpleNumMatches[0]))
    ) {
      return num;
    }
  }
...
const extractCodecDetails = (text) => {
  ...
  if (
    ...
    details.add("DTS-HD"); // General DTS-HD if not MA/HR
  else if (lowerText.includes("dts") && !lowerText.includes("dts-hd"))
    details.add("DTS"); // Plain DTS
  ...
  if (lowerText.includes("10bit") || lowerText.includes("10-bit"))
    details.add("10-bit");
  else if (lowerText.includes("8bit") || lowerText.includes("8-bit"))
    details.add("8-bit");
...
const sortStreamsByQuality = (streams) => {
  ...
  return [...streams].sort((a, b) => {
    ...
    const providerOrderA =
      providerSortKeys[providerA] || providerSortKeys.default;
    const providerOrderB =
      providerSortKeys[providerB] || providerSortKeys.default;
...
const mapRegionForPStream = (febboxRegion) => {
  ...
  console.log(
    `[PStream] Region ${febboxRegion} not found in mapping, using East as fallback`
  );
  return "East";
};

// Function to fetch streams from PStream API (new primary method)
const getStreamsFromPStreamAPI = async (
  imdbId,
  tmdbType,
  seasonNum = null,
  episodeNum = null,
  ...
  const timerLabel =
    `getStreamsFromPStreamAPI_${imdbId}_${tmdbType}` +
    (seasonNum ? `_s${seasonNum}` : "") +
    (episodeNum ? `_e${episodeNum}` : "");
  console.time(timerLabel);
  ...
  try {
    ...
    if (tmdbType === "movie") {
      ...
      console.log(
        `[PStream] Invalid parameters for ${tmdbType}: season=${seasonNum}, episode=${episodeNum}`
      );
      return [];
    }
    ...
    const pstreamRegion = mapRegionForPStream(
      regionPreference || global.lastRequestedRegion?.used
    );
    ...
    if (uiToken) {
      ...
      console.log(
        `[PStream] No ui-token available, making unauthenticated request`
      );
    }
    ...
    const useProxy =
      process.env.SHOWBOX_PROXY_URL_VALUE &&
      process.env.SHOWBOX_PROXY_URL_VALUE.trim() !== "";
    let finalApiUrl = apiUrl;

    if (useProxy) {
      finalApiUrl = `${process.env.SHOWBOX_PROXY_URL_VALUE}${encodeURIComponent(
        apiUrl
      )}`;
    ...
    const streams = processPStreamResponse(
      response.data,
      tmdbType,
      seasonNum,
      episodeNum
    ...
    if (error.response) {
      console.error(
        `[PStream] Status: ${error.response.status}, Data: ${JSON.stringify(
          error.response.data
        ).substring(0, 200)}`
      );
    }
...
const processPStreamResponse = (
  ...
  try {
    ...
    if (
      apiResponse &&
      apiResponse.streams &&
      typeof apiResponse.streams === "object"
    ) {
      console.log(
        `[PStream] Found streams object with ${
          Object.keys(apiResponse.streams).length
        } quality levels`
      );
        ...
        if (url && typeof url === "string") {
          ...
          const streamTitle =
            ...
            `${
              tmdbType === "movie"
                ? "PStream"
                : `PStream - S${seasonNum}E${episodeNum}`
            } - ${quality}`;
      ...
      console.log(
        `[PStream] Successfully parsed ${streams.length} streams from streams object`
      );
      return streams;
    }
    ...
    if (apiResponse && Array.isArray(apiResponse.sources)) {
      console.log(
        `[PStream] Found sources array with ${apiResponse.sources.length} sources`
      );
          ...
          const streamTitle =
            ...
            `${
              tmdbType === "movie"
                ? "PStream"
                : `PStream - S${seasonNum}E${episodeNum}`
            } - ${item.quality || item.label || "Unknown"}`;
...
const getTmdbDataForPStream = async (tmdbType, tmdbId) => {
  ...
  if (!externalIdsData || process.env.DISABLE_CACHE === "true") {
    ...
    console.log(
      `  [PStream] Fetching TMDB external IDs from: ${externalIdsApiUrl}`
    );
    ...
    try {
      ...
      console.log(
        `  [PStream] Error fetching TMDB external IDs: ${error.message}`
      );
    }
  }
...
const getUiTokenForPStream = async (userCookie = null) => {
  ...
  if (userCookie && userCookie.startsWith("eyJ")) {
    console.log(
      "[PStream CookieManager] Using user-provided JWT token as ui-token"
    );
    return userCookie;
  }
  ...
  if (userCookie && !userCookie.startsWith("eyJ")) {
    console.log(
      "[PStream CookieManager] Regular cookie provided, attempting to use as ui-token"
    );
  ...
  if (
    ...
    global.currentRequestUserCookie.startsWith("eyJ")
  ) {
  ...
  if (
    global.currentRequestConfig &&
    Array.isArray(global.currentRequestConfig.cookies) &&
    global.currentRequestConfig.cookies.length > 0
  ) {
    try {
      ...
      if (best && best.ok) {
        ...
        global.currentRequestUserCookieRemainingMB = best.remainingMB;
        console.log(
          `[PStream CookieManager] Selected best ui-token by remaining quota: ${best.remainingMB} MB`
        );
      ...
      console.warn(
        `[PStream CookieManager] Multi-cookie selection failed: ${e.message}`
      );
    }
  }
