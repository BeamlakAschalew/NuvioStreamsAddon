...
const selectBestCookie = async (cookies) => {
  ...
  if (cookieArray.length === 1) {
    ...
    global.currentRequestUserCookieRemainingMB = quotaResult.ok
      ? quotaResult.remainingMB
      : undefined;
    console.log(
      `[ShowBox] Using single cookie${
        quotaResult.ok
          ? ` (${quotaResult.remainingMB} MB remaining)`
          : " (quota check skipped/failed)"
      }`
    );
  ...
  if (successfulChecks.length > 0) {
    ...
    console.log(
      `[ShowBox] Selected best cookie by quota: ${best.remainingMB} MB remaining (out of ${successfulChecks.length} valid cookies)`
    );
  ...
  console.log(
    `[ShowBox] All quota checks failed, using first cookie as fallback`
  );
...
const getStreamsFromTmdbId = async (
  ...
  console.log(
    `[ShowBox] Getting streams for TMDB ${tmdbType}/${tmdbId}${
  ...
  try {
    ...
    const { cookie: selectedCookie, remainingMB } = await selectBestCookie(
      cookies
    );
    ...
    if (tmdbType === "tv" || tmdbType === "series") {
      if (seasonNum === null || episodeNum === null) {
        console.log(
          `[ShowBox] TV show requires both season and episode numbers`
        );
    ...
    // Add cookie as both header and query parameter if available
    if (selectedCookie) {
    ...
    console.log(
      `[ShowBox] Making request to: ${apiUrl.replace(
        /\?cookie=.*/,
        "?cookie=***"
      )}`
    );
      ...
      for (const version of apiData.versions) {
        ...
        if (version.links && Array.isArray(version.links)) {
          for (const link of version.links) {
            ...
            const streamQuality = parseQualityFromLabel(
              link.quality || link.name
            );
    ...
    console.log(
      `[ShowBox] Successfully parsed ${streams.length} streams from API response`
    );
