...
console.log(
  `[MoviesDrive] Internal cache is ${CACHE_ENABLED ? "enabled" : "disabled"}.`
);
const CACHE_DIR = process.env.VERCEL
  ? path.join("/tmp", ".moviesdrive_cache")
  : path.join(__dirname, ".cache", "moviesdrive");
...
const ensureCacheDir = async () => {
  ...
  try {
    ...
    console.error(
      `[MoviesDrive] Error creating cache directory: ${error.message}`
    );
  }
};
...
async function searchContent(query, callback) {
  ...
  try {
    ...
    if (cachedResults && cachedResults.length > 0) {
      console.log(
        `[MoviesDrive] Cache HIT for search: ${query}. Using ${cachedResults.length} cached results.`
      );
    ...
    if (cachedResults && cachedResults.length === 0) {
      console.log(
        `[MoviesDrive] Cache contains empty data for search: ${query}. Refetching from source.`
      );
    } else {
      console.log(
        `[MoviesDrive] Cache MISS for search: ${query}. Fetching from source.`
      );
    }
    ...
    function searchPage(page) {
      const searchUrl = `${mainUrl}/page/${page}/?s=${encodeURIComponent(
        query
      )}`;
      ...
      makeRequest(searchUrl, (err, html) => {
        ...
        if (pagesChecked < maxPages && movieElements.length > 0) {
          ...
          console.log(
            `[MoviesDrive] Caching ${searchResults.length} search results for: ${query}`
          );
          saveToCache(cacheKey, searchResults);
          callback(searchResults);
        }
...
function extractGDFlixLinks(url, title, callback) {
  ...
  makeRequest(
    ...
    (err, urlData) => {
      ...
      const newUrl = url
        .replace(/https:\/\/[^.]+\.gdflix\.[^/]+/, latestUrl)
        .replace(/https:\/\/gdlink\.[^/]+/, latestUrl);
...
function matchesEpisode(filename, episodeInfo) {
  ...
  const patterns = [
    ...
    new RegExp(
      `Season\\s*${episodeInfo.season}.*Episode\\s*${episodeInfo.episode}`,
      "i"
    ),
  ...
  downloadLinks.forEach((downloadLink) => {
    makeRequest(downloadLink, (err, pageHtml) => {
      ...
      streamingElements.each((index, element) => {
        ...
        if (
          href &&
          (href.toLowerCase().includes("hubcloud") ||
            href.toLowerCase().includes("gdflix") ||
            href.toLowerCase().includes("gdlink"))
        ) {
          let source = "Unknown";
          if (
            text.toLowerCase().includes("hubcloud") ||
            href.toLowerCase().includes("hubcloud")
          ) {
            source = "HubCloud";
          } else if (
            text.toLowerCase().includes("gdflix") ||
            href.toLowerCase().includes("gdflix")
          ) {
            source = "GDFlix";
          } else if (
            text.toLowerCase().includes("gdlink") ||
            href.toLowerCase().includes("gdlink")
          ) {
            source = "GDLink";
          }
      ...
      intermediateLinks.forEach((intermediate) => {
        if (intermediate.source === "HubCloud") {
          ...
          intermediate.source === "GDFlix" ||
          intermediate.source === "GDLink"
        ) {
...
async function extractStreamingLinks(url, callback, episodeInfo = null) {
  const episodeKey =
    episodeInfo && episodeInfo.isEpisode
      ? `_S${episodeInfo.season}E${episodeInfo.episode}`
      : "";
  const cacheKey = `download_links_${url.replace(
    /[^a-z0-9]/gi,
    "_"
  )}${episodeKey}`;
  ...
  try {
    ...
    if (cachedDownloadLinks && cachedDownloadLinks.length > 0) {
      console.log(
        `[MoviesDrive] Cache HIT for download links: ${url}. Using ${cachedDownloadLinks.length} cached download links.`
      );
    ...
    if (cachedDownloadLinks && cachedDownloadLinks.length === 0) {
      console.log(
        `[MoviesDrive] Cache contains empty data for download links: ${url}. Refetching from source.`
      );
    } else {
      console.log(
        `[MoviesDrive] Cache MISS for download links: ${url}. Fetching from source.`
      );
    }
    ...
    makeRequest(url, (err, html) => {
      ...
      console.log(
        `[MoviesDrive] Caching ${downloadLinks.length} download links for: ${url}`
      );
    ...
    console.error(
      `[MoviesDrive] Error in extractStreamingLinks:`,
      error.message
    );
    callback([]);
  }
...
function findStreamingLinks(query, callback) {
  ...
  if (episodeInfo.isEpisode) {
    ...
    searchQuery = query
      ...
      .trim();
    console.log(
      `Searching for series: "${searchQuery}" and filtering for S${episodeInfo.season
        .toString()
        .padStart(2, "0")}E${episodeInfo.episode.toString().padStart(2, "0")}`
    );
  }
  ...
  try {
    ...
    if (CACHE_ENABLED) {
      ...
      if (cachedData !== null) {
        ...
        // If cached data is empty or invalid, refetch
        if (
          !cachedData ||
          Object.keys(cachedData).length === 0 ||
          (!cachedData.title && !cachedData.name)
        ) {
          console.log(
            `[MoviesDrive] Cached TMDB data is empty or invalid, refetching: ${cacheKey}`
          );
...
function constructSearchQuery(
  ...
  // For TV shows, TMDB uses 'name', for movies it's 'title'
  let title =
    mediaType === "tv"
  ...
  if (!title) {
    console.error(
      "[MoviesDrive] No title found in metadata:",
...
async function getMoviesDriveStreams(
  ...
  try {
    ...
    if (!metadata) {
      console.log(
        `[MoviesDrive] Could not fetch metadata for TMDB ID: ${tmdbId}`
      );
      return [];
    }
